module.exports = function(crowi) {
  var mongoose = require('mongoose')
    ...
    , SECURITY_REGISTRATION_MODE_LDAP = 'Ldap'
  ;
  ...
  configSchema.statics.getRegistrationModeLabels = function()
  {
    ...
    labels[SECURITY_REGISTRATION_MODE_LDAP]       = 'LDAP (アプリからの新規登録不可)';
  ...
  configSchema.statics.hasLdapConfig = function(config)
  {
    if (!config.crowi['ldap:host'] ||
        //!config.crowi['ldap:port'] ||
        !config.crowi['ldap:bindDn'] ||
        !config.crowi['ldap:password']) {
      return false;
    }
    return true;
  };
  ...
  */
  ...
  Config.SECURITY_REGISTRATION_MODE_LDAP       = SECURITY_REGISTRATION_MODE_LDAP;
